      ...
      <div className="bg-white px-4 pt-12 pb-4 sticky top-0 z-10 border-b border-stone-200">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar">
          <button 
            ...
            className={`px-4 py-2 rounded-full whitespace-nowrap text-sm font-bold transition-colors ${filter === 'all' ? 'bg-brand-dark text-white' : 'bg-stone-100 text-stone-600'}`}
          >
            {t('enquiry.tab_all') || 'All'}
          </button>
          <button 
            onClick={() => setFilter('new')}
            className={`px-4 py-2 rounded-full whitespace-nowrap text-sm font-bold transition-colors flex items-center gap-2 ${filter === 'new' ? 'bg-brand-dark text-white' : 'bg-stone-100 text-stone-600'}`}
          >
            {t('enquiry.tab_new') || 'New'}
            {newCount > 0 && (
              <span className={`px-2 py-0.5 rounded-full text-xs ${filter === 'new' ? 'bg-white text-brand-dark' : 'bg-brand-dark text-white'}`}>
                {newCount}
              </span>
            )}
          </button>
          <button 
            ...
            className={`px-4 py-2 rounded-full whitespace-nowrap text-sm font-bold transition-colors ${filter === 'responded' ? 'bg-brand-dark text-white' : 'bg-stone-100 text-stone-600'}`}
          >
        ...
        {loading ? (
          ...
          filteredEnquiries.map(enq => (
            <div 
              ...
              className="bg-white rounded-2xl p-4 shadow-sm border border-stone-100 active:scale-[0.98] transition-transform cursor-pointer"
            >
